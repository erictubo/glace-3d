...
class RealFakeDataset(Dataset):
    ...
    def _load_image_set(self, idx):
        ...
        fake_mask = torch.from_numpy(fake_depth > 0).float().unsqueeze(0)
        diff_mask = torch.from_numpy(diff_depth > 0).float().unsqueeze(0)

        assert fake_mask.shape == fake_image.shape[:2], f"Mask shape {fake_mask.shape} does not match image shape {fake_image.shape[:2]}"
        assert diff_mask.shape == diff_image.shape[:2], f"Mask shape {diff_mask.shape} does not match image shape {diff_image.shape[:2]}"
    ...
    def _get_single_item(self, idx, image_height, angle=None):
        ...
        real_image, fake_image, diff_image, fake_mask, diff_mask = self._crop_to_smallest([real_image, fake_image, diff_image, fake_mask, diff_mask])
